...
6. For operationally relevant changes, keep `memory/progress.md` in `verifying` while checks are pending. `Status: done` is a completion claim, never proof; accept that claim only after applicable state-integrity, required-verification, Risk, attestation, and approval requirements pass. Keep one current Task when required, one declared Risk, immediate Next, explicit Blockers, and no more than five recent outcomes.
